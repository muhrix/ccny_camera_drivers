...
public:
    ...
    explicit AnalogControl(
        ...
        mvJSONSource(),
        mvStoreJSONCalibrationData(),
        mvLoadJSONCalibrationData(),
        mvJSONCalibrationFilename(),
        mvJSONCalibrationText()
    {
        mvIMPACT::acquire::DeviceComponentLocator locator( pDev, mvIMPACT::acquire::dltSetting, settingName );
        locator.bindSearchBase( locator.searchbase_id(), "Camera/GenICam" );
        ...
        locator.bindComponent( mvJSONSource, "mvJSONSource" );
        locator.bindComponent( mvStoreJSONCalibrationData, "mvStoreJSONCalibrationData@i" );
        locator.bindComponent( mvLoadJSONCalibrationData, "mvLoadJSONCalibrationData@i" );
        locator.bindComponent( mvJSONCalibrationFilename, "mvJSONCalibrationFilename" );
        locator.bindComponent( mvJSONCalibrationText, "mvJSONCalibrationText" );
    }
    PYTHON_ONLY( %immutable; )
    ...
    /// \brief Choses between file or text.
    /**
     *  Choses between file or text.
     */
    PropertyI64 mvJSONSource;
    /// \brief Stores calibration data.
    /**
     *  Stores calibration data.
     */
    Method mvStoreJSONCalibrationData;
    /// \brief Loads calibration data.
    /**
     *  Loads calibration data.
     */
    Method mvLoadJSONCalibrationData;
    /// \brief JSON filename to load and store from or to.
    /**
     *  JSON filename to load and store from or to.
     */
    PropertyS mvJSONCalibrationFilename;
    /// \brief JSON text to store or from load action.
    /**
     *  JSON text to store or from load from.
     */
    PropertyS mvJSONCalibrationText;
    PYTHON_ONLY( %mutable; )
#ifdef DOTNET_ONLY_CODE
    ...
    PropertyI64 getmvJSONSource( void ) const
    {
        return mvJSONSource;
    }
    Method getmvStoreJSONCalibrationData( void ) const
    {
        return mvStoreJSONCalibrationData;
    }
    Method getmvLoadJSONCalibrationData( void ) const
    {
        return mvLoadJSONCalibrationData;
    }
    PropertyS getmvJSONCalibrationFilename( void ) const
    {
        return mvJSONCalibrationFilename;
    }
    PropertyS getmvJSONCalibrationText( void ) const
    {
        return mvJSONCalibrationText;
    }
#endif // #ifdef DOTNET_ONLY_CODE
};
